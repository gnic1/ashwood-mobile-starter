...
// ---- Generic event logger (Ashwood flow) ----
// POST /api/event  { type: "chat"|"vision"|"image"|"stream", payload: {...} }
...
app.post("/api/chat", requireSecret, async (req, res) => {
  try {
    ...
    if (MOCK_MODE) {
      const output = "Proxy auth OK.";
      appendJson(DB_EVENTS, { id: `evt_${Date.now()}`, type: "chat", payload: { prompt, output }, at: new Date().toISOString() });
      return res.json({ ok: true, output, model: "mock" });
    }
...
// ---- Chat (streaming; client reads chunks) ----
app.post("/api/chat/stream", requireSecret, async (req, res) => {
  try {
    ...
    if (MOCK_MODE) {
      const mock = "Streaming mock... done.";
      res.write(mock);
      res.end();
      appendJson(DB_EVENTS, { id: `evt_${Date.now()}`, type: "stream", payload: { prompt, output: mock }, at: new Date().toISOString() });
      return;
    }
...
app.post("/api/vision", requireSecret, async (req, res) => {
  try {
    ...
    if (MOCK_MODE) {
      const output = "A mountain landscape with a lake.";
      appendJson(DB_EVENTS, { id: `evt_${Date.now()}`, type: "vision", payload: { image_url, prompt, output }, at: new Date().toISOString() });
      return res.json({ ok: true, output, model: "mock" });
    }
...
// ---- Image generation (also save file + metadata) ----
const normalizeSize = (size) => {
  const supported = new Set(["1024x1024","1024x1536","1536x1024","auto"]);
  if (supported.has(String(size))) return String(size);
  // smallest → map to 1024x1024
  return "1024x1024";
};
...
app.post("/api/image", requireSecret, async (req, res) => {
  try {
    ...
    if (MOCK_MODE) {
      const fake = Buffer.from("PNG").toString("base64");
      const id = `img_${Date.now()}`;
      const filename = `${id}.png`;
      const abs = path.join(IMAGES_DIR, filename);
      fs.writeFileSync(abs, Buffer.from(fake, "base64"));
      ...
      return res.json({ ok: true, image_base64: fake, size, image_url: rec.file, id });
    }
...
// POST /api/tag-image  { id: "img_...", tags: ["icon","logo"] }
