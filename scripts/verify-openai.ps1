...
try {
  ...
  $health = Invoke-WithRetry -Action {
    Invoke-RestMethod -Uri "$ServerUrl/health" -Headers $headers -Method Get -TimeoutSec 5
  } -Retries $retries
  Write-Host (ConvertTo-Json $health -Depth 5)

  Write-Step "Checking $ServerUrl/health/openai"
  $openaiHealth = Invoke-WithRetry -Action {
    Invoke-RestMethod -Uri "$ServerUrl/health/openai" -Headers $headers -Method Get -TimeoutSec 15
  } -Retries $retries
  Write-Host (ConvertTo-Json $openaiHealth -Depth 5)

  if ($mockMode) {
    Write-Host 'Proxy is running in MOCK_MODE=1; upstream OpenAI calls are simulated.' -ForegroundColor Yellow
  } elseif ($openaiHealth.ok) {
    Write-Host 'Proxy successfully reached OpenAI.' -ForegroundColor Green
  } else {
    Write-Host 'Proxy failed to reach OpenAI. Review the details above.' -ForegroundColor Red
  }
  ...
  if ($serverJob -and -not $LeaveServerRunning) {
    ...
    try {
      Stop-Job -Job $serverJob -Force -ErrorAction SilentlyContinue | Out-Null
    } catch {}
    try {
