...
try {
  ...
  if (-not $SkipInstall) {
    ...
    $p = Start-Process -FilePath "C:\Program Files\nodejs\npm.cmd" -ArgumentList "install" -NoNewWindow -PassThru -Wait
  ...
  if (-not (Test-Path $envPath)) { Write-Host "Creating server .env file at $envPath"; New-Item -ItemType File -Path $envPath -Force | Out-Null }
  ...
  if (-not $OpenAIApiKey -and -not $MockMode) { throw 'An OpenAI API key is required unless -MockMode is specified.' }

  Write-Step 'Writing environment configuration'
  if ($OpenAIApiKey) { Set-DotEnvValue -Path $envPath -Key 'OPENAI_API_KEY' -Value $OpenAIApiKey }
  elseif ($existingKey) { Set-DotEnvValue -Path $envPath -Key 'OPENAI_API_KEY' -Value $existingKey }
  ...
  if ($PSBoundParameters.ContainsKey('SharedSecret')) { Set-DotEnvValue -Path $envPath -Key 'SHARED_SECRET' -Value $SharedSecret }

  # PowerShell 5 compatible ternary replacement
  $mockValue = if ($MockMode.IsPresent) { '1' } else { '0' }
  Set-DotEnvValue -Path $envPath -Key 'MOCK_MODE' -Value $mockValue

  Set-DotEnvValue -Path $envPath -Key 'MAX_OUTPUT_TOKENS' -Value '512'

  Write-Step 'Summary'
  $installedMsg = if ($ranInstall) { 'yes' } else { 'no (skipped)' }
  Write-Host ("Server dependencies installed: {0}" -f $installedMsg) -ForegroundColor Green
  Write-Host "Environment file: $envPath" -ForegroundColor Green
  if ($MockMode) { Write-Host 'MOCK_MODE=1 (no real OpenAI calls will be made)' -ForegroundColor Yellow }
  else { Write-Host 'Real OpenAI API key configured.' -ForegroundColor Green }
