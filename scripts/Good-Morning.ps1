...
# --- Ensure .env exists with required keys (no commit) ---
$envPath = Join-Path $Root ".env"
if (-not (Test-Path $envPath)) {
    New-Item -ItemType File -Path $envPath | Out-Null
}

# Load current .env (if any)
$dotenv = @{}
...
# Ask for API key if missing or placeholder
...
# Ensure model + base-url defaults (buttons use ChatGPT via proxy)
...
if (Test-Path $adb) {
    ...
    Write-Warning "adb.exe not found; continuing. (Buttons should still work if your emulator connects over localhost.)"
}

# --- Start Proxy in a separate PowerShell window ---
try {
    Get-Process node -ErrorAction SilentlyContinue | Stop-Process -Force
} catch {}
$proxyCmd = "cd `"$Root`"; node proxy\server.js"
...
# --- Start Expo in this window (localhost mode) ---
